...
/**
 * Records that an agent chose to keep working past their scheduled time out.
 *
 * The choice itself is not restricted — somebody finishing a call at 5:02 is not
 * doing anything wrong, and a system that locked them out mid-call would be
 * worse than one that lets them finish. What this does is make sure the terms
 * are stated before the hours are worked rather than discovered afterwards: the
 * notification says plainly that time past the scheduled end is not paid unless
 * it was approved in advance.
 *
 * It lands in the agent's own notifications, which is a record they keep and can
 * re-read — an on-screen dialog is gone the moment it is dismissed. The
 * acknowledgement is also written to the activity log, so if the hours are ever
 * disputed there is a timestamped entry showing what was shown and when.
 */
export async function acknowledgeUnpaidOvertimeAction() {
  const { user, db } = await requireUserLite();
  const today = todayInTz();
  const record = db.attendance.find((a) => a.user_id === user.id && a.work_date === today);

  // Nothing to acknowledge if they are not on the clock, or already off it.
  if (!record || !record.time_in || record.time_out) return;

  const scheduledTimeOut = record.scheduled_time_out || db.work_schedule.work_end;

  // Once a day. The dialog can be raised again by a refresh, and an agent who
  // works two hours over should not collect a notification for each one.
  const already = db.notifications.some(
    (n) => n.recipient_id === user.id && n.type === "unpaid_overtime" && (n.created_at || "").slice(0, 10) === today
  );
  if (already) return;
  ...
  notify(
    ...
    `Your shift was scheduled to end at ${scheduledTimeOut}. You have chosen to continue working. ` +
      `Please note that hours worked beyond your scheduled time out are not compensated unless overtime ` +
      `was approved in advance by your Team Lead or Management. If this work is required, request approval ` +
      `so the hours can be recorded and paid correctly.`,
    "/attendance/clock"
  );

  logActivity(db, user.id, "UNPAID_OVERTIME_ACKNOWLEDGED", "attendance", record.id, {
    scheduled_time_out: scheduledTimeOut,
    acknowledged_at: nowIso(),
  }, { module: "attendance", ...(await getRequestInfo()) });
